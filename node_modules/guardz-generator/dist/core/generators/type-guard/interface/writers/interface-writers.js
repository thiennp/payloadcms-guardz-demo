...
const writeGenericInterfaceTypeGuard = (interfaceDecl, guardName, collectionResult, allInterfaces, sourceFiles, importMap) => {
    ...
    return `export const ${guardName} = <${typeParamString}>(${genericParams}): TypeGuardFn<${interfaceDecl.name.text}<${typeParamNamesOnly.join(', ')}>> =>
  isType<${interfaceDecl.name.text}<${typeParamNamesOnly.join(', ')}>>({ ${propertyGuards} });`;
};
