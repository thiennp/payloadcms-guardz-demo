...
const copyOriginalImports = (context) => {
    ...
    for (const originalImport of originalImports) {
        (0, logging_1.log)(`[copyOriginalImports] Processing original import: ${originalImport.typeName} (${originalImport.importType}, isDefault: ${originalImport.isDefault})`);
...
const removeUnusedImports = (allImports, typeGuardCode) => {
    ...
    for (const importItem of allImports) {
        ...
        for (const importName of importNames) {
            // Check if the imported item is actually used in the type guard code
            // Use a more precise approach to avoid substring matches
            // This prevents "isType" from matching "Type" and similar issues
            const escapedName = importName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            // Use a regex that ensures we match the exact import name as a whole word
            // but also handles cases where it might be part of a larger identifier
            // Updated to handle generic type parameters like <AxiosResponse>
            const nameIsUsed = new RegExp(`(?:^|[^a-zA-Z0-9_])${escapedName}(?:[^a-zA-Z0-9_]|$)|<${escapedName}>`).test(typeGuardCode);
            if (nameIsUsed) {
                isUsed = true;
                break;
            }
...
const executeImportStrategy = (importTypes, usedEnums, usedGuardzUtilities, usedTypeGuards, usedGuardzTypeAliases, needsTypeGuardFnImport, needsTypeGuardFnConfigImport, context, buildTypeImportInfo, buildTypeGuardImportInfo, typeGuardCode) => {
    ...
    const originalImports = context.currentSourceFile && context.importMap
        ? (0, import_extractors_1.extractImportsFromFile)(context.currentSourceFile)
        : [];
    (0, logging_1.log)(`[executeImportStrategy] Step 2: Found ${originalImports.length} original imports from source file`);
    for (const originalImport of originalImports) {
    ...
    const filteredImports = removeUnusedImports(allImports, typeGuardCode || '');
    (0, logging_1.log)(`[executeImportStrategy] Step 7: Total imports after filtering: ${filteredImports.length}`);
    const finalImports = filteredImports.filter(i => i.path != null);
    (0, logging_1.log)(`[executeImportStrategy] Step 7: Final imports with valid paths: ${finalImports.length}`);
    const result = {
        imports: finalImports,
        hasEnums,
    };
    (0, logging_1.log)(`[executeImportStrategy] Output: ${result.imports.length} imports, hasEnums = ${result.hasEnums}`);
    return result;
};
